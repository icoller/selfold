 ...
 * @LastEditors: coller
 * @LastEditTime: 2023-12-24 17:32:15
 * @Desc:
 */
package repo
...
func (r *ArticleRepo) ListByCategoryIDNotId(ctx *context.Context, id, categoryId int) (res []entity.ArticleBase, err error) {
	err = db.DB.Model(&entity.ArticleBase{}).Scopes(gormx.Context(ctx, gormx.WhereCategoryIdNotId(id, categoryId))).Find(&res).Error
	return
}
